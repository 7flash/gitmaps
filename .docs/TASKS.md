...
- [x] ~~**Preview mode should show scroll/thumb + diff markers**~~ — ✅ DONE. Preview cards now draw a right-side scrollbar thumb plus added/deleted diff markers inside the canvas preview so scroll position and change locations remain visible. Follow-up polish increased the diff rail width, marker size, whole-file changed-state visibility, fixed preview pills to prefer commit diff data when resolving marker sources, added a real clickable DOM marker overlay, and made the preview scrollbar track/thumb interactive via click + drag.
