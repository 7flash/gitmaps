...
- [x] ~~**Preview mode controls/settings simplification**~~ — ✅ DONE. Simplified the Settings modal by removing the popup-preview font slider, replacing the confusing preview near/far title controls with one fixed preview text size control, keeping line-density controls as zoomed-out/zoomed-in line targets, disabling hover popup previews by default, adding `Ctrl+=` / `Ctrl+-` / `Ctrl+0` preview text sizing shortcuts, moving Settings into the bottom zoom pill, and making Simple the default control mode at runtime.
...
- [x] ~~**Auto directory/shared-prefix containers should be removable**~~ — ✅ DONE. Disabled the auto-generated directory labels / shared-prefix containers on the canvas because they were adding noise without helping navigation.
