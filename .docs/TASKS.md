...
- [x] ~~**Preview mode controls/settings simplification**~~ — ✅ DONE. Simplified the Settings modal by removing the popup-preview font slider, renaming preview controls to zoomed-out/zoomed-in wording instead of mode wording, disabling hover popup previews by default, and making Simple the default control mode at runtime.
- [x] ~~**Layer move should not refresh the page**~~ — ✅ DONE. Card context menu actions now use explicit `type="button"` buttons so moving a file to another layer no longer behaves like a form submit / page refresh.
