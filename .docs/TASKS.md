...
- [ ] **Shared canvas context lifecycle cleanup** — Audit mount/cleanup paths now that `app/lib/context.ts` exposes `getCanvasContext()` / `setCanvasContext()` for UI entrypoints that trigger repo reloads.
