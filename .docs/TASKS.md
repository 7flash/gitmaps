...
- [ ] **Initial route bootstrap helper split** — Consider separating the remaining UI bootstrap callback (`loadSavedPositions`, shared layout, layers, viewport restore) into a tiny helper so `page.client.tsx` sheds more orchestration code.
