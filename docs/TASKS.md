...
- [x] ~~**Canvas preview wrap no longer preserves whole words or injects ellipses**~~ — ✅ DONE. `low-zoom-preview.ts` now wraps by raw character chunks in canvas cards and clips over-measured lines without adding `...`/`…`.
